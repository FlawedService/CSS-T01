...
public class SaleTransactionScripts {
	 ...
	 * @param employeeId The identification of the employee. This @param was created by @author fc44312
	...
	public int newSale (int vat, int employeeID) throws ApplicationException {
		try {
			...
			if (employeeID != 0){
				//Execpçao se o employee nao existir
				...
				// creates the sale
				SaleRowDataGateway newSale = new SaleRowDataGateway(customer.getCustomerId(), new Date(), employeeID);
				newSale.insert();
				return newSale.getId();
			}
			System.out.println(" The employee ID is not Valid! ");
			return 0;
	...
	/**
	 * Gets a SaleRowGateway object for a 	statement.setInt(3, manager_id);given sales id, or raises a 
	...
	private SaleRowDataGateway getSale(int saleId) throws ApplicationException {
		try {
			...
			throw new ApplicationException("Sale with id: " + saleId + " does not exist.", e);
		}
	}
	...
	/**
	 * Delivery of a sale to given customer
	 * 
	 * @param saleid
	 * @return new Delivery
	 * @throws ApplicationException 
	...
	public void saleDelivery(int saleid, String address) throws PersistenceException, ApplicationException, SQLException {
		...
		if (sale.getStatus() == SaleStatus.CLOSED)
			throw new ApplicationException(" Cannot delivery to closed sales.");
		
		DeliveryRowDataGateway newDelivery = new DeliveryRowDataGateway(saleid,sale.getCustomerId(),address);
	...
	public Set<SaleRowDataGateway> pendente() throws SQLException, PersistenceException{
		return SaleRowDataGateway.pendingsales();
	}
