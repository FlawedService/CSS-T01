...
public class CustomerRowDataGateway {
	...
	private static final String INSERT_CUSTOMER_SQL = 
			"insert into customer (id, vatnumber, designation, phonenumber, discount_id, address ) " +
			"values (DEFAULT, ?, ?, ?, ?, ?)";
	...
	private static final String GET_CUSTOMER_BY_VAT_NUMBER_SQL = 
			   "select id, vatnumber, designation, phonenumber, discount_id, address " +
					"from customer " +
					"where vatnumber = ?";
	...
	public CustomerRowDataGateway(int vat, String designation, int phoneNumber,
			DiscountType discountType, String address) {
		this.vat = vat;
	...
	/**
	 * The all address a customer has by customerId SQL statement
	 */
	private static final String GET_ALL_ADDRESS_BY_CUSTOMER_ID = 
			   "select address " +
					"from customer " +
					"where id = ?";
	
	public static CustomerRowDataGateway getAddress(int id) throws SQLException, PersistenceException {
		//ArrayList<String>
		try(PreparedStatement statement = DataSource.INSTANCE.prepare(GET_ALL_ADDRESS_BY_CUSTOMER_ID)){
			...
			throw new PersistenceException("Error na morada", e);
		}
	}
	...
	private static CustomerRowDataGateway loadCustomer(ResultSet rs) throws RecordNotFoundException {
		try {
			...
			CustomerRowDataGateway newCustomer = new CustomerRowDataGateway(rs.getInt("vatnumber"), 
					...
					toDiscountType(rs.getInt("discount_id")), rs.getString("address"));
	...
	public static Set<CustomerRowDataGateway> getAllAddress (int saleId) throws PersistenceException {
		try(PreparedStatement statement = DataSource.INSTANCE.prepareGetGenKey(GET_ALL_ADDRESS_BY_CUSTOMER_ID)){
	...
	private static Set<CustomerRowDataGateway> loadAllAddress(ResultSet rs) throws SQLException{
		Set<CustomerRowDataGateway> result = new HashSet<CustomerRowDataGateway>();
		while(rs.next()){
			CustomerRowDataGateway address= new CustomerRowDataGateway(rs.getInt("vatnumber"), 
					rs.getString("designation"), rs.getInt("phonenumber"), 
					toDiscountType(rs.getInt("discount_id")), rs.getString("address"));
